...
#error "Please define a way to initialize a memory block."
    ...
    if (time_in_secs(total_time) > 0)
        ee_printf("Iterations/Sec   : %d\n",
                  default_num_contexts * results[0].iterations
                      / time_in_secs(total_time));
#endif
    /* (Removed 10-second warning for fast RTL simulation) */
    ...
    if (total_errors == 0)
    {
        ...
        if (known_id == 3)
        {
            uint32_t cm_per_sec = (uint32_t)( ((uint64_t)default_num_contexts * results[0].iterations * EE_TICKS_PER_SEC) / total_time );
            uint32_t mhz = EE_TICKS_PER_SEC / 1000000;
            uint32_t cm_per_mhz_int = cm_per_sec / mhz;
            uint32_t cm_per_mhz_frac = ((cm_per_sec % mhz) * 1000) / mhz;

            ee_printf("\n");
            uint32_t cycles_per_iter = total_time / (default_num_contexts * results[0].iterations);
            ee_printf("Cycles/Iteration:        %d\n", cycles_per_iter);
            ee_printf("CoreMark/MHz:            %d.%03d\n", cm_per_mhz_int, cm_per_mhz_frac);
            ee_printf("Iterations/Sec:          %d\n", cm_per_sec);
            ee_printf("Compiler Version:        %s\n", COMPILER_VERSION);
            ee_printf("Compiler Flags:          %s\n", COMPILER_FLAGS);
            ee_printf("\n");

            // Restore the standard CoreMark benchmark string which MUST report Iterations/Sec
